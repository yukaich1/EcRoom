...
| 发布作品 | 用户愿意把这版内容展示在个人主页 | 生成或更新 `published_post` | 进入发布编辑页，整理成帖子 |
...
发布页是一个正式页面，不使用小弹窗承载完整编辑。它的路径可以是 `/publish/{post_id}`，用于把“创作结果”整理成“可展示帖子”。
...
| 标签 | 产品内置标签 + 用户自定义 | 是 | 不由 Agent 自动检索或自动决定 |
...
- 系统只提供规范的默认标签池，例如：角色、世界观、社媒、改稿、标题、活动、游戏、品牌、剧情、宣发；
- 用户可以点击默认标签，也可以手动输入自定义标签；
- 标签去重、限制数量、允许删除；
...
分类切换只改变素材集合，不改变创作系统的泛用性。用户可以从任意分类进入生成，最终仍由需求理解和技能流决定具体创作路径。
...
```text
需求理解 -> 资料检索 -> 创作策略 -> 初稿写作 -> 改稿整理 -> 质量评审 -> 规范检查 -> 记忆沉淀
```
